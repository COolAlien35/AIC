...
def main() -> None:
    ...
    bench_src = (repo / "results_final") if (repo / "results_final").exists() else (repo / "results")
    ...
    _copy_if_exists(repo / "VIDEO_SCRIPT.md", sub / "VIDEO_SCRIPT.md")
    _copy_if_exists(repo / "DESIGN.md", sub / "DESIGN.md")
    _copy_if_exists(repo / ".env.example", sub / ".env.example")
    _copy_if_exists(repo / "COLAB_GPU_RUNBOOK.md", sub / "COLAB_GPU_RUNBOOK.md")
    _copy_if_exists(repo / "train_colab.ipynb", sub / "train_colab.ipynb")
    _copy_if_exists(repo / "pyproject.toml", sub / "pyproject.toml")
    _copy_if_exists(repo / "requirements.txt", sub / "requirements.txt")

    # HF Space env (the URL judges pull)
    _copy_if_exists(repo / "hf_env_space", sub / "hf_env_space")
    _copy_if_exists(repo / "scripts" / "deploy_hf_env_space.sh", sub / "scripts" / "deploy_hf_env_space.sh")

    # Auxiliary scripts judges may want to run
    for script in [
        "openai_baseline.py",
        "score_tasks.py",
        ...
        _copy_if_exists(repo / "scripts" / script, sub / "scripts" / script)

    # Tasks (3 graders + registry)
    _copy_if_exists(repo / "aic" / "tasks", sub / "aic_tasks")
    ...
    # Benchmarks (raw + normalized + task-grader)
    for p in [
        bench_src / "benchmark_summary.csv",
        bench_src / "benchmark_by_scenario.csv",
        bench_src / "statistical_test.json",
        bench_src / "benchmark_summary_normalized.csv",
        bench_src / "benchmark_by_scenario_normalized.csv",
        bench_src / "benchmark_by_task_grader.csv",
        bench_src / "normalized_score_manifest.json",
        bench_src / "inference_stdout.log",
        bench_src / "openai_baseline_scores.json",
        bench_src / "openai_baseline_episodes.csv",
        bench_src / "inference_summary.json",
    ]:
        _copy_if_exists(p, sub / "benchmarks" / p.name)

    # Training-evidence (NOTE 1 hard requirement)
    for name in [
        ...
        _copy_if_exists(repo / "results" / name, sub / "training_evidence" / name)
    _copy_if_exists(repo / "logs" / "grpo_progress.jsonl", sub / "training_evidence" / "grpo_progress.jsonl")
    ...
    _write_json(
        sub / "manifest.json",
        {
            ...
            "submission_target": "Meta OpenEnv hackathon",
            "hf_space_env_url": "https://huggingface.co/spaces/KINGKK007/aic-openenv-env",
            "github_url": "https://github.com/COolAlien35/AIC",
            "contains": {
                "spec": [
                    "openenv.yaml", "Dockerfile", "README.md", "inference.py",
                    "VIDEO_SCRIPT.md", "DESIGN.md", ".env.example",
                    "COLAB_GPU_RUNBOOK.md", "train_colab.ipynb",
                    "pyproject.toml", "requirements.txt",
                ],
                "hf_env_space": ["hf_env_space/*", "scripts/deploy_hf_env_space.sh"],
                "scripts": [
                    "scripts/openai_baseline.py",
                    "scripts/score_tasks.py",
                    "scripts/plot_grpo_progress.py",
                ...
                "benchmarks": [
                    ...
                    "benchmarks/inference_stdout.log",
                ],
                "training_evidence": [
                    "training_evidence/grpo_reward_curve.png",
                    "training_evidence/grpo_loss_curve.png",
                    "training_evidence/grpo_kl_curve.png",
                    "training_evidence/grpo_training_summary.json",
                    "training_evidence/grpo_progress.jsonl",
                    "training_evidence/openenv_validate.log",
                    "training_evidence/hf_space_smoke.log",
                ],
                "plots": ["plots/*.png"],
            },
        },
